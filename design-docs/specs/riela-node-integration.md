...
`web-gateway` will later run as a node inside `tacogips/riela` workflows,
...
- input: `{ "url": string, "count": number, "learnIfMissing": bool }`
  mapped to `web-gateway fetch <url> --count N --learn-if-missing`
...
5. Config injectable via `--config` / `WEB_GATEWAY_CONFIG`, DB path via
   `--db` / `WEB_GATEWAY_DB`, so parallel nodes can isolate state.
6. Library alternative: `WebGateway` is constructible from an in-memory
